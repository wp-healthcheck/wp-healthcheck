...
<?php if ( wphc()->core()->wordpress()->get_core_auto_update_option() !== false ) : ?>
	<?php
	...
	<p class="wphc_wp_auto_update">
		<?php foreach ( $wp_update_options as $option => $description ) : ?>
			<span><label><input id="wphc-opt-wp-updates" name="wphc-opt-wp-updates" type="radio" value="<?php echo $option; ?>" <?php echo ( wphc()->core()->wordpress()->get_core_auto_update_option() === $option ) ? 'checked="checked"' : ''; ?> /> <?php echo $description; ?></label></span>
		<?php endforeach; ?>
	</p>
