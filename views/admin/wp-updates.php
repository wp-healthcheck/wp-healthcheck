...
<?php if ( wphc( 'wordpress' )->get_auto_update_policy() !== false ) : ?>
	<?php
	...
	<p class="wphc_wp_auto_update">
		<?php foreach ( $wp_update_options as $option => $description ) : ?>
			<span><label><input id="wphc-opt-wp-updates" name="wphc-opt-wp-updates" type="radio" value="<?php echo $option; ?>" <?php echo ( wphc( 'wordpress' )->get_auto_update_policy() === $option ) ? 'checked="checked"' : ''; ?> /> <?php echo $description; ?></label></span>
		<?php endforeach; ?>
	</p>
