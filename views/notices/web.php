...
$server_status = wphc( 'module.server' )->is_updated( $service );

if ( $server_status === 'updated' || $server_status === false ) {
	return false;
}
...
<div class="notice wphc-notice wphc-notice-web <?php echo esc_attr( $messages[ $server_status ]['class'] ); ?>">
	<p>
		<strong><?php esc_html_e( 'WP Healthcheck:', 'wp-healthcheck' ); ?></strong>
		<?php echo wp_kses_post( $messages[ $server_status ]['message'] ); ?>
	</p>
</div>
